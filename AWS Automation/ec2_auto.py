...
# Defining the function for deleting the older data
...
class Ec2Instance(object):
    ...
    def delete_available_volumes(self):
        ...
        for volume in volumes:
            if volume['State'] == 'available':
                self.ec2.delete_colume(VolumeId = volume['VolumeId'])
    ...
    def shutdown(self):
        ...
        for res in instances['Reservations']:
            for instance in res['Instance']:
                ...
                if is_unassigned(tags):
                    print('Instance to terminate ' + instance['InstanceId'])
                    instance_to_terminate.append(instance['InstanceId'])
        ...
        if any(instance_to_stop):
            self.ec2.stop_instance(InstanceIds = instance_to_stop)

        if any(instance_to_terminate):
            print(instance_to_terminate)
            self.ec2.terminate_instances(InstanceIds = instance_to_terminate)

if __name__ == '__main__':
    ec2 = Ec2Instance('us-east-1')
